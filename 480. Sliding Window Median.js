...
var medianSlidingWindow = function(nums, k) {
    ...
    nums = nums.map((val, i) => {
        ...
        return k % 2 ? minHeap.peek().val : 
        (minHeap.peek().val - maxHeap.peek().val) / 2
    })
    return nums.slice(k - 1)
};
function HeapMap() {
        let heap = [], map = new Map()
        function Node(id, val) {
            this.id = id
            this.val = val
        }
        function swap(i, j) {
            const id1 = heap[i].id, id2 = heap[j].id
            map.set(id1, j);
            map.set(id2, i);
        ...
        this.push = function(id, val = id) {
            const i = heap.length, node = new Node(id, val)
            heap.push(node)
            map.set(id, i)
            bubbleUp(i)
            return node
        ...
        this.set = function(id, val) {
            if(!map.has(id)) return false
            const i = map.get(id), node = heap[i]
            node.val = val
            bubbleUp(i)
            bubbleDown(i)
            return node
